// Source: https://github.com/openmaptiles/fonts
...
var todo = [];
fs.readdirSync('./fonts/input/').forEach(function(dir,a) {
  if (fs.lstatSync(path.resolve('./fonts/input', dir)).isDirectory()) {
    var fonts;
    try {
      ...
      fonts = [];
      fs.readdirSync(path.resolve('./fonts/input', dir)).forEach(function(file) {
...
todo.reduce(function(p, pair) {
    return p.then(function() {
      ...
      return doFonts(path.resolve('./fonts/input', pair[0]), pair[1]);
    });
  }, Promise.resolve()
).then(function() {
